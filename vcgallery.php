...
/**
 * Plugin Name: Easy Gallery Management
 * Plugin URI: https://wordpress.org/plugins/vc-gallery/
 * Description: Displays a gallery in WordPress with password protection, Masonry layout, and options to share and download gallery images.
 * Version: 1.0
 * Author: ViitorCloud
 * Author URI: http://viitorcloud.com/
 * License: GPLv2
 *
 * @package Easy_Gallery_Management
 */
...
function VCGL_welcome_screen_function() {
	...
	<div class="wrap about-wrap">
				...
				foreach ( $setting_tabs_wc as $name => $label ) {
					echo '<a  href="' . home_url( 'wp-admin/index.php?page=welcome-vcgallery&tab=' . $name ) . '"
                        class="nav-tab ' . ( $current_tab_wc == $name ? 'nav-tab-active' : '' ) . '">' . $label . '</a>';
				}
				?>
				</h2>
